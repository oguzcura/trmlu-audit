...
Both a named preprint version and an anonymous review version are provided as
LaTeX source (PDFs are generated by compiling — they are build artifacts, not
tracked in git):

| File | Description |
|------|-------------|
| [`paper_preprint.tex`](paper/paper_preprint.tex) | **Recommended** — named, camera-ready source (author: *Oğuz Emre Cura*, `oguzemrecura@gmail.com`, `github.com/oguzcura`). Compile for arXiv or workshop submission. |
| [`paper.tex`](paper/paper.tex) | Anonymous **review** source (no author name — for double-blind peer review; `\usepackage[review]{acl}` is already set). |
| [`custom.bib`](paper/custom.bib) | Verified references (real author names, live-fetched from arXiv). |

**Compile a PDF** (requires `tectonic` or another XeTeX engine):

```bash
cd paper
tectonic paper_preprint.tex    # -> paper_preprint.pdf (named preprint)
tectonic paper.tex             # -> paper.pdf (anonymous review version)
```
...
analyzer so the n=175 numbers are fully re-derivable by running the scripts in
this repository (generated `results/` outputs are gitignored to keep the repo lean).
